...
def milesdistance(a1, a2):
    # return 0
    return milesdistance(a1, a2)

loc_cache = {}
def getlocation(address):
    if address in loc_cache: return loc_cache[address]
    url = 'http://nominatim.openstreetmap.org/search?q=%s&format=xml' % address
    print url
    data = urlopen(url).read()
    doc = parseString(data)

    lat = doc.getElementsByTagName('place')[0].getAttributeNode('lat').nodeValue
    lon = doc.getElementsByTagName('place')[0].getAttributeNode('lon').nodeValue
...
def milesdistance(a1, a2):
    ...
    londif = 53.0 * (lon2 - lon1)
    return (latdif ** 2 + londif ** 2) ** .5
...
def scaledata(rows):
    ...
    # find the lowest and hightest values
    for row in rows:
        d = row.data
    ...
    def scaleinput(d):
        return [(d[i] - low[i]) / (high[i] - low[i]) for i in 
                range(len(low))]
